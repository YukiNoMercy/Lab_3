...
class TTrain:
    def __init__(self, N): #конструктор. Инициализация класса. Обязательно надо.
        ...
        self.indexs = [] #массив для хранения индексов пустых вагонов
    ...
    def serch_empty(self): #Метод для поиска пустых купе
        k = 0 #Переменная для сохранения индекса пустого купе
        for i in self.wagon: # Цикл, который перебирает элементы массива(То есть мы в и храним словарь(купе))
            k += 1 #При переборе нового купе увеличиваем наш индекс пустого купе на 1
            ...
            if flag: #Если у нас в переменной flag лежит True, То мы увеличиваем значение переменной,
                # которая отвечает за подсчёт пустых вагонов на 1 и добавляем в массив индексов пустых купе индекс нового пустого купе
                self.count_empty += 1
